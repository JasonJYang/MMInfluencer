...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        self.model.train()
        total_loss = 0
        for batch_idx, (head_index, rel_type, tail_index) in enumerate(self.train_data_loader):
            self.optimizer.zero_grad()
            loss = self.model.transe.loss(head_index.to(self.device), rel_type.to(self.device), tail_index.to(self.device))
            loss.backward()
            self.optimizer.step()
